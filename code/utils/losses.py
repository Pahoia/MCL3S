...
def uncertainty_loss(inputs, targets):
    ...
    pseudo_label = F.softmax(targets / 0.5, dim=1).detach()  # Shape: [batch_size, num_classes, h, w, d]

    # Convert pseudo_label to class indices
    pseudo_label_indices = torch.argmax(pseudo_label, dim=1)  # Shape: [batch_size, h, w, d]

    # Compute cross-entropy loss
    vanilla_loss = F.cross_entropy(inputs, pseudo_label_indices, reduction='none')  # Shape: [batch_size, h, w, d]

    # uncertainty rectification
    kl_div = torch.sum(
        F.kl_div(F.log_softmax(inputs, dim=1), F.softmax(targets, dim=1).detach(), reduction='none'), dim=1
    )
