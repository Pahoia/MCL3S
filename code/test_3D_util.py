...
def test_all_case(net, base_dir, method="unet_3D", test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160),
                  stride_xy=18, stride_z=4, test_save_path=None):
    with open(base_dir + '/{}'.format(test_list), 'r') as f:
        image_list = f.readlines()
    image_list = [os.path.join(base_dir, item.strip(), 'mri_norm2.h5') for item in image_list]
    ...
    with open(test_save_path + "/{}.txt".format(method), "a") as f:
        for image_path in image_list:
            ids = os.path.basename(os.path.dirname(image_path))  # 获取文件夹名作为 ID
            print(ids)
            ...
            sitk.WriteImage(pred_itk, os.path.join(test_save_path, "{}_pred.nii.gz".format(ids)))

            img_itk = sitk.GetImageFromArray(image)
            img_itk.SetSpacing((1.0, 1.0, 1.0))
            sitk.WriteImage(img_itk, os.path.join(test_save_path, "{}_img.nii.gz".format(ids)))

            lab_itk = sitk.GetImageFromArray(label.astype(np.uint8))
            lab_itk.SetSpacing((1.0, 1.0, 1.0))
            sitk.WriteImage(lab_itk, os.path.join(test_save_path, "{}_lab.nii.gz".format(ids)))

        f.writelines("Mean metrics,{},{},{},{}".format(
            total_metric[0, 0] / len(image_list),
            total_metric[0, 1] / len(image_list),
            total_metric[0, 2] / len(image_list),
            total_metric[0, 3] / len(image_list)))
...
def calculate_metric_percase(pred, gt):
    if not np.any(gt) or not np.any(pred):
        print("Warning: Either ground truth (gt) or prediction (pred) contains no foreground objects.")
